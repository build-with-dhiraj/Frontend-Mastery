...
import { pageEntrance } from "../lib/dhiraj-motion";
import {
  DhirajCard,
  DhirajScrollReveal,
  DhirajTextReveal,
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-[rgb(var(--dhiraj-line))] bg-[rgb(var(--dhiraj-bg)_/_0.86)] backdrop-blur">
      <div className="mx-auto flex h-14 max-w-7xl items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
        <a href="/" className="text-sm font-semibold tracking-normal text-[rgb(var(--dhiraj-fg))]">
          {brand}
        </a>
    ...
    <section className="px-4 pb-14 pt-14 sm:px-6 sm:pt-20 lg:px-8">
      <div className="mx-auto grid max-w-7xl items-center gap-10 lg:grid-cols-[1.02fr_0.98fr]">
        <div>
          ...
          {proof ? (
            <p className="mt-6 text-sm text-[rgb(var(--dhiraj-muted))]">{proof}</p>
          ) : null}
        </div>
        {media ? (
    ...
    <div className="grid gap-4 md:grid-cols-3">
      {items.map((item, index) => (
        ...
        >
          ...
          <p className="mt-6 text-xs text-[rgb(var(--dhiraj-muted))]">
    ...
    <div className="grid gap-4 lg:grid-cols-3">
      {plans.map((plan) => (
        <DhirajCard
          key={plan.name}
          interactive
          className={cn(plan.highlighted && "border-[rgb(var(--dhiraj-blue))]")}
        >
    ...
    <DhirajScrollReveal>
      <div className="rounded-[var(--dhiraj-radius-md)] border border-[rgb(var(--dhiraj-line))] bg-[rgb(var(--dhiraj-fg))] px-6 py-10 text-[rgb(var(--dhiraj-bg))] sm:px-8">
        <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
          ...
          <a
            href={action.href}
            className="inline-flex min-h-10 items-center justify-center rounded-[var(--dhiraj-radius-md)] bg-[rgb(var(--dhiraj-bg))] px-4 text-sm font-medium text-[rgb(var(--dhiraj-fg))]"
          >
            {action.label}
          </a>
        </div>
...
export function DhirajTestimonial({
  quote,
  name,
  role,
}: {
  ...
  return (
    <DhirajCard>
      <blockquote className="text-lg leading-8 text-[rgb(var(--dhiraj-fg))]">
        "{quote}"
      </blockquote>
    ...
    <footer className="border-t border-[rgb(var(--dhiraj-line))] px-4 py-10 sm:px-6 lg:px-8">
      <div className="mx-auto flex max-w-7xl flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
        ...
        <nav className="flex flex-wrap gap-x-4 gap-y-2 text-sm text-[rgb(var(--dhiraj-muted))]">
          {links.map((link) => (
            <a key={link.href} href={link.href} className="hover:text-[rgb(var(--dhiraj-fg))]">
              {link.label}
            </a>
          ))}
        </nav>
